"""Unit tests for area chart component."""
...
from src.marketdata.visualization.components.area_chart import (
    AreaChart,
    AreaStyle
)
...
def chart():
    ...
    return AreaChart(dimensions, scales, style)
...
def test_chart_initialization(chart):
    ...
    assert chart.style.fill_color == (0.2, 0.6, 1.0)
    assert chart.style.fill_opacity == 0.2
    assert chart.style.line_color == (0.2, 0.6, 1.0)  # Should default to fill_color
    ...
    assert not chart.style.use_gradient
    assert chart.style.gradient_stops is None
    assert chart.value_column == 'close'
    assert chart.base_value == 0.0
...
def test_render_with_gradient(chart, renderer, sample_data):
    """Test rendering with gradient fill."""
    chart.style.use_gradient = True
    chart.style.gradient_stops = [
        (0.0, (0.2, 0.6, 1.0, 0.4)),
        (1.0, (0.2, 0.6, 1.0, 0.0))
    ]
    chart.set_data(sample_data)
...
def test_render_with_different_styles(chart, renderer, sample_data):
    ...
    # Change colors
    chart.style.fill_color = (1.0, 0.0, 0.0)
    chart.style.line_color = (0.0, 0.0, 1.0)
    chart.render(renderer)
    
    # Change line properties
    chart.style.line_width = 3.0
    chart.style.fill_opacity = 0.4
    chart.render(renderer)
...
def test_render_with_different_base_values(chart, renderer, sample_data):
    """Test rendering with different base values."""
    chart.set_data(sample_data)
    
    # Test different base values
    for base_value in [0.0, 50.0, 100.0]:
        chart.base_value = base_value
        chart.render(renderer)
