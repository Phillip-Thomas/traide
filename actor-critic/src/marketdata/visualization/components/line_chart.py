...
class LineStyle:
    ...
    line_color: Tuple[float, float, float] = (0.2, 0.6, 1.0)  # Blue
    line_width: float = 2.0
    show_points: bool = False
    point_size: float = 4.0
    smooth: bool = False
    fill_alpha: float = 0.1  # 0.0 for no fill
...
class LineChart(BaseChart):
    ...
    def _draw_grid(self, renderer: Renderer) -> None:
        ...
        price_range = visible_data[self.value_column].max() - visible_data[self.value_column].min()
        num_lines = 5
        price_step = price_range / num_lines
        
        for i in range(num_lines + 1):
            price = visible_data[self.value_column].min() + i * price_step
    ...
    def _draw_line(self, renderer: Renderer) -> None:
        """Draw the line.
        ...
        if self.style.fill_alpha > 0:
            ...
            fill_points.append((
                points[-1][0],
                self.dimensions.height - self.dimensions.margin_bottom
            ))
            fill_points.append((
                points[0][0],
                self.dimensions.height - self.dimensions.margin_bottom
            ...
            # Draw fill
            fill_color = self.style.line_color + (self.style.fill_alpha,)
